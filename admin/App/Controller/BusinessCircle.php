...
class Controller_BusinessCircle extends FLEA_Controller_Action {
	...
	function actionIndex() {
		...
		$sql="select province.name as province,city.name as city,town.name as town,circle.* from ".$prefix."business_circle circle 
			left join ".$prefix."address_province province on circle.province_id=province.id
			left join ".$prefix."address_city city on circle.city_id=city.id
			left join ".$prefix."address_town town on circle.town_id=town.id where ".$conditions;
